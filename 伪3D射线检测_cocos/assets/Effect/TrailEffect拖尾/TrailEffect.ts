...
export default class TrailEffect extends cc.Component {
     ...
     */
    public setShape(close: boolean = false) { // polygon: cc.Vec2[], 
    ...
    public update(dt: number) {
        // cc.log(this.playing + "gx" + this.target);
        if (!this._playing)
            this.moveTo(this.oldPos);
    ...
    @property(cc.Node)
    protected gensui: cc.Node;
    // 初始化
    public onLoad() {

        this.init(); // 初始化
        // cc.log("长度" + this.polygon.length);
        this.setShape();
        this.setTarget(this.gensui, cc.v3(0, 0, 0));
        this.play(cc.v3(0, 0, 0));
        // cc.log("拖尾初始化完成,长度" + this.polygon.length);
    }
}
